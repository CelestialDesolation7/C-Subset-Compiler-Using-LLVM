...
└─ runLinearScan()                                   // 5. 执行线性扫描分配
   ...
   └─ for each interval:
      ...
      ├─ handleActiveInactiveTransitions(start)      //    5.3 active↔inactive 转换
      ├─ allocatePhysicalReg(interval)               //    5.4 分配物理寄存器
      │  └─ insertActiveInterval(&interval)          //        插入 active 列表
      └─ spillAtInterval(interval)                   //    5.5 溢出处理（active+inactive）
...
与早期版本的主要区别：
- `runLinearScan()` 不再接受 `intervals` 参数，直接使用成员变量 `intervals_`（以便代码生成器通过 `getIntervals()` 访问）
- 每个区间处理时，在 `expireOldIntervals` 之后增加了 `handleActiveInactiveTransitions` 调用
...
从空闲寄存器池中取出优先级最高的寄存器，将区间映射到该物理寄存器，并按结束位置有序插入 `active_` 列表。
...
### handleActiveInactiveTransitions() — 区间空洞感知
...
1. **Active → Inactive**：`active` 中在当前位置 `curStart` 处进入空洞（`inHole(curStart)` 返回 true）的区间，移至 `inactive`。不释放其物理寄存器——空洞只是暂时的，区间在后续位置会恢复活跃。

2. **Inactive → Expired**：`inactive` 中已完全结束（`end() < curStart`）的区间，释放物理寄存器并移除。

3. **Inactive → Active**：`inactive` 中在当前位置恢复活跃（`contains(curStart)` 返回 true）的区间，移回 `active`（按 `end()` 升序插入）。

**`inHole()` 判定**：当位置落在区间的整体范围 `[start(), end()]` 内，但不被任何一段 `LiveRange` 覆盖时，说明该位置位于区间的空洞中。

```
区间 y 的 ranges: [0,5]  [12,17]
                  ████    ████
位置 8 → inHole = true（在 [5,12] 的间隙中）
位置 3 → inHole = false（被 [0,5] 覆盖）
```

### spillAtInterval() — 溢出处理（同时考虑 active 和 inactive）
...
早期版本仅从 `active` 列表中选择溢出候选。新版本**同时扫描 `active` 和 `inactive` 两个列表**，因为 `inactive` 中的区间虽然暂时处于空洞但仍占用物理寄存器。从全局（active+inactive）选择结束位置最晚的区间进行溢出，可以更好地最大化寄存器利用率。

```
溢出前:
  active:   [a0: ████████████████] (结束很晚)
  inactive: [a3: ██    ██████████] (结束更晚，处于空洞)
  当前:     [??:     ████]        (结束较早)
  空闲:     (无)

溢出 inactive 中 a3 后:
  active:   [a0: ████████████████], [a3:     ████]
  溢出:     [栈: ██    ██████████] (原 inactive 溢出到栈)
```
