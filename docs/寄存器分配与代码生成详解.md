...
1. **计算活跃区间**：每个虚拟寄存器的活跃范围（可含多段 `LiveRange`，即存在空洞）
2. **按 start 排序**：所有区间按起始位置排序
3. **顺序扫描**：维护 `active`（当前占用物理寄存器的区间）和 `inactive`（物理寄存器被占但在当前处于空洞的区间），对每个新区间：
   - 释放已过期的旧区间（`expireOldIntervals`）
   - **Active/Inactive 转换**（`handleActiveInactiveTransitions`）：将 `active` 中进入空洞的区间移至 `inactive`；将 `inactive` 中恢复活跃的区间移回 `active`；释放已完全结束的 `inactive` 区间
   - 如有空闲寄存器 → 分配
   - 否则 → 溢出（从 `active` + `inactive` 中选择结束最晚的候选，或溢出当前区间）
...
当物理寄存器耗尽时，采用**贪心策略**，同时考虑 `active` 和 `inactive` 两个列表中的候选：

```
在 active + inactive 中找到结束位置最晚的区间 bestSpill
if bestSpill 结束位置比当前区间更晚:
    溢出 bestSpill（将其物理寄存器让给当前区间）
else:
    溢出当前区间
```

与经典算法仅考虑 `active` 列表不同，本实现在溢出时**同时扫描 `inactive` 列表**，因为 `inactive` 中的区间虽然暂时处于空洞但仍占用着物理寄存器。选择全局结束最晚的区间溢出，可以最大化物理寄存器的释放效果。
...
callSaveSize_    = 对每个 call 指令：统计在该调用点活跃的 caller-saved 寄存器数量 → 取最大值 × 4
callArgAreaSize_ = 函数内所有 call 指令 → max(0, 参数数 - 8) × 4 的最大值
```

其中 `callSaveSize_` 的计算基于**活跃性感知**：对函数中的每个 `call` 指令，通过 `interval.contains(callPosUse)` 查询该调用位置上仍然活跃的 caller-saved 寄存器集合，只统计真正需要保存的寄存器数量，而非函数级别的所有 caller-saved 寄存器。这种逐调用点计算然后取最大值的策略既保证了正确性，又有效缩减了栈帧空间。
...
| 区间列表 | 仅 `active` | `active` + `inactive`（空洞感知） |
| 状态转换 | 无 | `handleActiveInactiveTransitions()` 处理区间空洞 |
| 溢出决策 | 仅从 active 选择候选 | 从 active + inactive 选择结束最晚的 |
| Caller-saved 保存 | 保存所有 caller-saved | 按需保存（仅保存调用点活跃的） |
