...
2. **`callSaveSize_`**（caller-saved 保存区大小）— 基于**活跃性感知**计算：对每个 `call` 指令，通过 `interval.contains(callPosUse)` 查询该调用点上仍然活跃的 caller-saved 寄存器数量，然后取所有调用点的最大值 × 4，而非统计函数级别的所有 caller-saved 寄存器。这有效缩减了栈帧空间。
...
### genCall — 函数调用（活跃性感知 caller-saved 按需保存）
...
void RISCVCodeGen::genCall(const Instruction &inst) {
    ...
    // —— 仅保存在当前调用点仍然活跃的（排除溢出临时寄存器和 def 寄存器）
    std::vector<int> savedRegs;
    for (auto &[vreg, physReg] : alloc.vregToPhys) {
        if (regInfo_.isCallerSaved(physReg) &&
            !funcAllocators_[currentFunction_]->isSpillTempReg(physReg) &&
            physReg != defPhysReg) {
            // 通过活跃区间查询该 vreg 在调用点是否活跃
            auto ivIt = intervals.find(vreg);
            if (ivIt != intervals.end() && ivIt->second->contains(callPosUse)) {
...
**1. 活跃性感知的 Caller-saved 按需保存（与早期版本的核心差异）**

早期版本无条件保存所有被分配到 caller-saved 寄存器的 vreg，新版本通过活跃区间查询 `interval.contains(callPosUse)` 仅保存**在调用点仍然活跃的寄存器**。例如，如果一个变量在 `call` 之前就已经死亡（最后一次使用早于调用位置），其 caller-saved 寄存器无需保存和恢复，可以减少不必要的 `sw`/`lw` 指令。

```
早期版本（保存所有）:                  新版本（按需保存）:
  sw a0, 0(sp)   # ← a0 已死亡，浪费       ✔ 跳过（a0 在调用点不活跃）
  sw a1, 4(sp)   # ← a1 仍活跃，必要       sw a1, 0(sp)   # a1 仍活跃
  call foo                                call foo
  lw a0, 0(sp)   # ← 多余恢复             ✔ 跳过
  lw a1, 4(sp)   # ← 必要恢复             lw a1, 0(sp)
```
...
| caller-saved 保存区 | `callSaveSize_` | genCall 按需保存活跃的 caller-saved 寄存器（基于活跃区间查询） |
